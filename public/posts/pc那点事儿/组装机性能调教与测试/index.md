...
5. 关闭TSME
   ...
   TSME(Transparent Secure Memory Encryption)，是AMD推出的一项内存加密技术，默认会加密全部内存，防止物理硬件级别的内存渗透攻击。会牺牲内存运行速度，换来一定程度的安全性。
...
{{< admonition >}}
上面写的自动超频方法，后续经我测试并不具备普适性，且会导致CPU发热量较大。推荐采用下面的这一手动全核负压方法，对于9700X来说普适性比较强。
{{< /admonition >}}

1. 进入AMD Overclocking，找到Precision Boost Overdrive，设置为Advanced模式
2. 将PBO Limits设置为Motherboard
3. 将PBO Scalar Control设置为Manual，PBO Scalar设置为1X
4. 进入Curve Optimizer，选择All Core
5. 将All Core Optimizer Sign设置为Negative
6. 将All Core Optimizer Magnitude设置为25
...
到这一步，所有BIOS调教优化设置就都结束了。按下<kbd>F10</kbd>可以保存设置并重启，应用新的BIOS设置。
